...
namespace woodpecker
{
  ...
  public:
    ...
    ~woodpecker_controller()
    {
      if (!config_mode)
      {
        *this << ">>> TOTAL PASSED: " << total << " OUT OF " << maximum << " ( " << total / maximum * 100 << "%)" << "\n";
      }
      if (test_log.is_open())
      {
        test_log.close();
...
/**
 * @brief Equality checking, passes if val == expr is true. Only evaluates val
 * == expr if no checks failed in the test so far.
 *
...
#define CHECK_CLOSE_TO(val, expr, relative_tolerance)                         \
  ...
  {                                                                           \
    ...
    {                                                                         \
      ...
      cntr.ok =                                                               \
          error <= ABSOLUTE_TOLERANCE + (relative_tolerance) * std::abs(val); \
      if (!cntr.ok)                                                           \
      {                                                                       \
        cntr.print_logs();                                                    \
